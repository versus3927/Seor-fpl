...
При `STAFF_ONLY_COMMANDS=true` slash-команды доступны только владельцу и участникам со служебными ролями. Канал `команды` также скрыт от остальных. Кнопки очереди и пользовательские панели продолжают работать.
