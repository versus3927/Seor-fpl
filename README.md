...
4. Включи **Server Members Intent**. Voice State intent доступен по умолчанию.
...
3. Бот создаст категории четырёх лиг, каналы `ranked`, голосовые `Lobby 1`, категорию приватных комнат и панель настройки.
...
7. После матча нажмите **Отправить результат** или используйте `/result`.
...
| `/result match_id` | Отправить результат матча |
...
- Для быстрой регистрации slash-команд укажи `GUILD_ID`. Без него команды синхронизируются глобально и могут появляться до часа.
